...
const CitizenDashboard = ({ user, onLogout }) => {
  ...
  // Check if user is citizen or user, redirect if not
  useEffect(() => {
    console.log('CitizenDashboard: user prop =', user, 'type:', typeof user);
    if (user !== "user" && user !== "citizen") {
      console.log('CitizenDashboard: User is not a citizen, redirecting to home');
      navigate("/");
    }
  }, [user, navigate]);
